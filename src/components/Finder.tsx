...
import {locationData, LocationNode, searchLocations, getBreadcrumbs, locationMap} from '../data/locationData';
...
const Finder: React.FC = () => {
    ...
    const [visibleLevels, setVisibleLevels] = useState<[number, number, number]>([0, 1, 2]);

    // Stav pre položky v každom stĺpci
    const [columnItems, setColumnItems] = useState<[LocationNode[], LocationNode[], LocationNode[]]>([[], [], []]);
    ...
    // Tu si pomozeme uz nastavenymi breadcrumbami a nebudeme filtrovat locationData
    const selectedColumnItems = useMemo<[LocationNode | null, LocationNode | null, LocationNode | null]>(() => {
        return [
    ...
    const updateVisibleLevels = useCallback((node: LocationNode) => {
        ...
        // Posúvame okno alebo zobrazujeme dalsie okno, iba ak má uzol deti
        if (node.children.length > 0) {
            ...
            // Ak je to uzol úrovne 1 (kraj), zobrazíme úrovne 0, 1, 2
            else if (nodeLevel === 1) {
                setVisibleLevels([0, 1, 2]);
            }
            // Ak je to uzol úrovne 2 (okres), zobrazíme úrovne 1, 2, 3
            else if (nodeLevel === 2) {
    ...
    // Hook pre aktualizáciu stĺpcov na základe vybraného uzla a viditeľných úrovní
    useEffect(() => {
        // Vyčistíme stĺpce
        ...
        if (!selectedNode) {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                if (targetLevel === selectedNode.level) {
                    ...
                    if (parentNode) {
                        // Ak má rodiča, zobrazíme všetky jeho deti (súrodencov vybraného uzla)
                        newColumnItems[i] = parentNode.children.filter(node => node.level === targetLevel);
                    } else {
                        // Ak nemá rodiča (je to krajina), zobrazíme všetky krajiny
                        newColumnItems[i] = locationData.filter(node => node.level === targetLevel);
                    }
                    ...
                    const ancestorId = selectedNode.path[targetLevel]; //fyi da sa to aj cez parentId v tomto pripade
                    const ancestor = ancestorId ? locationMap[ancestorId] : null;

                    if (ancestor) {
                        // Ak sme našli predka, zobrazíme jeho súrodencov a to tak ze zistime jeho rodiča a zobrazime jeho deti
                        const ancestorParentId = ancestor.parentId;
                        const ancestorParent = ancestorParentId ? locationMap[ancestorParentId] : null;

                        if (ancestorParent) {
                            // Ak má rodiča, zobrazíme jeho deti co su nasi surodenci
                            newColumnItems[i] = ancestorParent.children.filter(node => node.level === targetLevel);
                        } else if (targetLevel === 0) {
                            // Ak je to úroveň 0 (krajiny), zobrazíme všetky krajiny
                            newColumnItems[i] = locationData.filter(node => node.level === 0);
                        }
                    }
                } else if (targetLevel > selectedNode.level) {
                    // Ak je úroveň stĺpca vyššia ako úroveň vybraného uzla, zobrazíme jeho deti

                    newColumnItems[i] = selectedNode.children.filter(node => node.level === targetLevel);
                }
            }
        }
        ...
        (columnIndex: ColumnIndex, node: LocationNode) => {
            ...
            if (node.children.length > 0) {
                ...
                if (columnIndex === (2 as ColumnIndex)) {
                    ...
                    const nodeLevel = node.level;
                    const newVisibleLevels: [number, number, number] = [Math.max(0, nodeLevel - 1), nodeLevel, nodeLevel + 1];
    ...
    return (
        <Box className={styles.finderContainer}>
            ...
            <Box className={styles.columnsContainer}>
                {/* Prvý stĺpec sa zobrazuje, len ak má položky */}
                ...
                {/* Druhý stĺpec sa zobrazuje, len ak má položky */}
                ...
                {/* Tretí stĺpec sa zobrazuje, len ak má položky */}
